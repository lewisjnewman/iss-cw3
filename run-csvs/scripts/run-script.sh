...
docker run -d --security-opt label:type:docker_webserver_t --security-opt seccomp=../runs/webserver/final_seccomp_nginx.json --security-opt no-new-privileges --net iss-cw3_net --ip 192.0.2.10 --hostname webserver --add-host database:192.0.2.3 -p 80:80 --name webserver_c webserver_si
